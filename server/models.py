...
def save_directory_path(instance, filename):
    ...
    return '{0}/{1}/{2}'.format("server_hw", instance.code, filename)
...
class ServerHardware(models.Model):
    ...
    def get_absolute_url(self):
        ...
        """
        return reverse('server_hw_edit', args=[str(self.pk)])

    def get_detail(self):
        """
        Handy way of getting the url of the object to its detail view page
        """
        return reverse('server_hw_detail', args=[str(self.pk)])

    def get_delete(self):
        """
        Handy way of getting the url of the object to its delete view page
        """
        return reverse('server_hw_delete', args=[str(self.pk)])

    def get_class(self):
        """
        Handy way of getting the class of the object for the html templates
        """
        class_name = "Server avadanlığı"
        return class_name
    ...
    class Meta:
        verbose_name_plural = "Server avadanlıqları"
